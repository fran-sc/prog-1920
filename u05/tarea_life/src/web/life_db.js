...
function LifeGame() {
    ...
    this.init = function(rows, cols, ncells) {
        ...
        this.buffer1 = this.create_matrix(rows, cols, 0);
        this.buffer2 = this.create_matrix(rows, cols, 0);
        this.data = this.buffer1;
        this.next = this.buffer2;
    ...
    this.evolve = function() {
        // limpia el segundo buffer
        this.fillMatrix(this.next, 0);
    ...
    // Rellena la matriz 2D al valor indicado
    this.fillMatrix = function(matrix, value) {
        for(let r=0; r<matrix.length; r++)
            for(let c=0; c<matrix[0].length; c++)
                matrix[r][c] = value;
    }
    
    // Intercambia los punteros a las matrices del doble buffer
    this.swap = function() {
        let temp = this.data;
        this.data = this.next;
        this.next = temp;
