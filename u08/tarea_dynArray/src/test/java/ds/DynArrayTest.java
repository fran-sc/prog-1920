...
public class DynArrayTest {
    ...
    public void test_Constr() {
        DynArray<Integer> dynArr = new DynArray<>();    // Creamos el array dinámico 
    ...
    public void test_Constr_20() {
        DynArray<Integer> dynArr = new DynArray<>(20);  // Creamos el array dinámico 
        assertEquals(dynArr.size(), 0);                 // Comprobamos ocupación =0
        assertTrue(dynArr.isEmpty());                   // Comprobamos vacío =true
    }
    ...
    @Test 
    // Test 21: test iterador de for-each
    public void test_for_each() {
        String[] testset = {"cero", "uno", "dos"};
        String output = "cerounodos";

        DynArray<String> dynArr = new DynArray(testset); // Creamos el array dinámico con los datos de prueba

        for(String s: dynArr) System.out.print(s);

        assertEquals(output, outContent.toString());   // Comprobamos la salida
    }

    //-----------------------------------------------------------------------------------------------
    // Utility methods

    private<T> boolean compareArr(DynArray<T> dyn, T[] arr) {
        for(int i=0; i<arr.length; i++)
            if(!dyn.get(i).equals(arr[i])) return false;
        return true;
    }

    private<T> boolean compareDynArr(DynArray<T> dynArr1, DynArray<T> dynArr2) {
