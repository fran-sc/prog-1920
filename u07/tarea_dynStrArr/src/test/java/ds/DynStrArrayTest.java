...
public class DynStrArrayTest {
    ...
    public void test_Constr_20() {
        DynStrArray dynArr = new DynStrArray(20);   // Creamos el array dinámico 
        assertEquals(dynArr.size(), 0);             // Comprobamos ocupación =0
        assertTrue(dynArr.isEmpty());               // Comprobamos vacío =true
    }
    ...
    private boolean compareArr(DynStrArray dyn, String[] arr) {
        for(int i=0; i<arr.length; i++)
            if(!dyn.get(i).equals(arr[i])) return false;
        return true;
    }

    private boolean compareDynArr(DynStrArray dynArr1, DynStrArray dynArr2) {
